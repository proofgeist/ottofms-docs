...
After entering this information, click "Save" to save the configuration. OttoFMS will test the connection and if successful, will create a new offsite location in the OttoFMS console. If the test fails, you will be prompted to correct the information and try again.

<Callout type="info">
  Offsite Backups with OttoFMS require the following S3 permissions:
...
- `PutObject`
- `PutObjectACL`
- `CreateBucket` (optional after 4.11.0)
...
If you have additional Container Data Folders, they can be included in the backup to your offsite storage. Simply toggle the "Include External Container Data Folder" toggles on the schedule for whichever folders you want to include in your backups and they will be included going forward. When including the external container data folders, OttoFMS will push the contents of external folders to a new folder `container1` or `container2` in the remote path. This folder is at the same level as the `dbs` folder the database backups are put into. Files that are removed on the server are not deleted from the offsite location.

In OttoFMS 4.13.0 and later, external container folders can be synced to a different offsite location than the databases. This is ideal if you want to keep the data separate, store them in different storage types, or have different lifecycle rules for the two types of data. To send to a different offsite lcoation, toggle on the pertinent option and enter the new location details.
...
In version 4.11.0, OttoFMS exposes the option to send notifications when Offsite backups complete. You can set up a single notification destination per server, which can be a webhook or a Slack channel. For details on payloads and setup, see the [Notifications](/ottofms-features/notifications) documentation.
