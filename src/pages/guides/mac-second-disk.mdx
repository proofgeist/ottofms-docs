...
### Ensure OttoFMS is enabled

You may see the OttoFMS executable already in the list. If so, make sure it is enabled. If not, click the + button and add the OttoFMS executable. It is located in the `OttoFMS/bin/` folder in your Applications folder.

- Depending on the version of OttoFMS, the name of the executable will be either `ottofms-macos` or `ottofms-macos-x64` or `ottofms-mac-arm64`.

### [Restart Otto](/managing-ottofms/starting-stopping#starting-stopping-and-restarting)
...
If you upgrade OttoFMS or FileMaker Server or MacOS, and suddenly deployments stop working with a permission error, it is likely that OttoFMS does not have access to the backup disk.
