...
Download for [Linux](/downloads/ottofms/4.14.1/amd), [Linux (arm)](/downloads/ottofms/4.14.1/arm), [Windows](/downloads/ottofms/4.14.1/win), [Mac](/downloads/ottofms/4.14.1/mac), [Mac (Apple Silicon)](/downloads/ottofms/4.14.1/mac-arm)


### Patch Changes

- Fixed an issue where newly created data api keys would not be properly loaded into the register webhook modal.
- Fixed an error where errors during scheduled deployment start would crash OttoFMS.
- Reverted a change from 4.14.0 which caused an incorrect server id to be used for license validation, breaking offline activations.
- Fixed an issue where an unknown parsing error would fail a deployment when there was a OttoFMS build (non-Just in Time) in a sub-deployment before a sub-deployment using a Just In Time build.

## 4.14.0

Released: 11/24/2025

Download for [Linux](/downloads/ottofms/4.14.0/amd), [Linux (arm)](/downloads/ottofms/4.14.0/arm), [Windows](/downloads/ottofms/4.14.0/win), [Mac](/downloads/ottofms/4.14.0/mac), [Mac (Apple Silicon)](/downloads/ottofms/4.14.0/mac-arm)

### Minor Changes

- OttoFMS now exposes a UI to work with FileMaker webhooks (which we're referring to as Triggers in the UI to prevent confusion). These can be used to trigger webhook notifications when records in a specific table in a FileMaker file change. The OttoFMS UI exposes a feature that is available in FileMaker Server, but does not have its own UI at this time.

### Patch Changes

- Upgraded to Node.js 22.
- The OData MCP tool creation/edit modal now displays a helpful error if OttoFMS is unable to fetch scripts from the file.
- The OData MCP tool creation/edit modal now displays the odata url that it is sending to when testing tools.
- When creating a MCP Server in OttoFMS, you will now be immediately redirected to the MCP server's dashboard instead of having to exit the success modal and clicking into the dashboard manually
- Added a warning in the Tool Modal for scripts that have names that will not work with OData (scripts containing special characters).
- The OData MCP calls now respect the PREVENT_DAPI_CALLOUT env variable.
- Fixed an issue where the submit button in the MCP tool form would be disabled in some cases when it shouldn't have.
- When editing the input schema for MCP tools, new parameters will be focused automatically.
- Fixed an issue where updates and deletions on MCP Tools would not be represented in MCP clients until OttoFMS was restarted.
- Added column filtering for the builds table.
- Fixed an issue where the schedule information in the scheduled deployment tab of the deployment page would not display.
- Fixed possible errors when checking the license when the FileMaker Server APIs are not working properly.
- The pagination and sorting options on the build table are now stored to the browser to persist across page reloads.
- Allow Save As XML option with only file credentials (no server-level Admin API key required).
- The Data API and OData Proxies now pass along all headers when making calls.
- The OttoFMS OData Proxy now allows using Basic authentication as well as Data API Authentication.
- OttoFMS will now list the container data folders in the disk info stats.
- Scheduled deployments will now fail to start if the deployment it attempts to trigger is not scheduled.
- OttoFMS no longer stores build file credentials in the manifest.json by default. You can opt into storing the credentials in the manifest using the includeCredentialsInManifest boolean flag in the start build API. Storing the credentials in the manifest is primarily useful when using a build as a publicly accessible build.
- Fixed an issue where the JIT build timeout override was not being used.
- Fixed an issue where backups created by OttoFMS would fail if the filename had a trailing space.
- Attempting to view a log file that does not exist will now return a not found error rather than a generic server error.
- If the selected log in the log viewer does not exist, OttoFMS will now automatically switch to viewing an existing log file instead.

## 4.13.1

Released: 10/7/2025

Download for [Linux](/downloads/ottofms/4.13.1/amd), [Linux (arm)](/downloads/ottofms/4.13.1/arm), [Windows](/downloads/ottofms/4.13.1/win), [Mac](/downloads/ottofms/4.13.1/mac), [Mac (Apple Silicon)](/downloads/ottofms/4.13.1/mac-arm)

### Patch Changes

- Fixed an issue where OttoFMS would fail to start on new installs (starting with 4.13.0) due to a sqlite table being missing.

## 4.13.0

Released: 10/6/2025

Download for [Linux](/downloads/ottofms/4.13.0/amd), [Linux (arm)](/downloads/ottofms/4.13.0/arm), [Windows](/downloads/ottofms/4.13.0/win), [Mac](/downloads/ottofms/4.13.0/mac), [Mac (Apple Silicon)](/downloads/ottofms/4.13.0/mac-arm)

### Minor Changes

- OttoFMS has a new Apps page, which displays FileMaker Apps you can install onto your server using OttoFMS. This page replaces the OttoDeploy and SimpleQ popups that had previously appeared in the console, so they and the settings managing them have been removed.
- OttoFMS now exposes MCP Servers using OData. These servers can be set up on the MCP Server tab of the OttoFMS dashboard. OData MCP servers allow you to set up Tools to be calls by AI Agents which can call scripts in your FileMaker solution, allowing you to expose functionality with AI for users outside of your FileMaker application. One MCP server maps to one file, and authentication is handled with Data API Keys (OData access required). One tools maps to one script, and the output of the script is passed back to the LLM as its response. Check out the docs for more details!
- OttoFMS Offsite Backup Notifications can now be configured to not send notifications on success. Default behavior remains the same, notifications are sent on success.
- OttoFMS Offsite Schedules can now have different remotes/buckets/paths set up for external container data folders, allowing you to set up different lifecycle rules or storage types for external container data.

### Patch Changes

- Added a more useful error message for mismatched folders when doing a deployment using the developer tool.
- Removed the "Test" step of Offsite location creation since OttoFMS does the same test when the location is created.
- Fixed an issue where OttoFMS debug logging would not start immediately on startup, causing some logs not to be written to the otto-debug.log (specifically the custom CA cert debugging logs were missing).
- Validation errors on OttoFMS endpoints now have more helpful error messages.
- Fixed an issue where the folder size would not populate in the file browser until you changed folders.

## 4.12.0

Released: 9/15/2025

Download for [Linux](/downloads/ottofms/4.12.0/amd), [Linux (arm)](/downloads/ottofms/4.12.0/arm), [Windows](/downloads/ottofms/4.12.0/win), [Mac](/downloads/ottofms/4.12.0/mac), [Mac (Apple Silicon)](/downloads/ottofms/4.12.0/mac-arm)

### Minor Changes

- Exposed a Developer API and UI for saving a file as compressed.
- Added the ability to add text files to the .build.zip so that they can be shipped with builds.
- Added support for encrypting, decrypting, and saving as compressed in place, which does not require selecting a target file or file path and handles the renaming of the new file to the correct filename.
- OttoFMS can now Clone closed files when creating a build.
- Added ability to store pre-deployment and post-deployment script information on a build.
- OttoFMS now exposes an Admin MCP server at /otto/admin-mcp. This mcp server lets you use AI agents to manage your filemaker server and files. You can get server stats and settings, manage and download filemaker files, debug/rerun/undo deployments, and debug/rerun builds for your server.
- OttoFMS Webhooks can now accept PATCH and DELETE requests. If you include the headers in the webhook request, the request method will be added to the headers as the "X-Method" header.

### Patch Changes

- Fixed an error when some errors in endpoints could throw an "invalid character in statusMessage" error rather than the actual error that occurred.
- When setting up notifications for a build or offsites on a server that has never run a deployment, the url in the slack notifications and webhook response will now have the correct URL.
- Fixed an issue with the File Browser where it would throw a "Invalid String Length" error when listing folders with thousands of files.
- Added the open files to the clients list in the OttoFMS console.
- Fixed an issue where the data api key list in the webhook creation flow would overflow the screen if there were too many data api keys.
- Added logs for encrypting, decrypting, and saving xmls of files.
- Fixed an error where uploads would fail if the file-store.config.json file was corrupted or completely empty.
- The "PREVENT_DAPI_CALLOUT" environment variable now also prevents callout on webhooks.
- Fixed an issue with Deployment and Build Script file management on Windows.
- Reduced latency for data api and odata proxies.
- Fixed an issue where OttoFMS still required the S3 CreateBucket permission when testing an offsite location connection.
- Removed unnecessary callouts to the server fqdn when creating data api keys.
- The link to the source server in the deployment page now opens to a new tab.
- Fixed an issue where the OttoFMS web server config could conflict with other updates made after OttoFMS was installed.

## 4.11.1

Released: 6/26/2025

Download for [Linux](/downloads/ottofms/4.11.1/amd), [Linux (arm)](/downloads/ottofms/4.11.1/arm), [Windows](/downloads/ottofms/4.11.1/win), [Mac](/downloads/ottofms/4.11.1/mac), [Mac (Apple Silicon)](/downloads/ottofms/4.11.1/mac-arm)

### Patch Changes

- Offsite notifications can now be removed from a server.
- Offsite notification settings are now editable from the OttoFMS Developer API.
- When uploading a file and renaming it, you can now choose to overwrite an existing file of the same name.
- When downloading a clone of a file, the downloaded file will now have "\_clone" at the end of its filename.
- OttoFMS now allows you to download a single file from the top toolbar of the file browser even if you cannot download multiple files from that location (ie. if you are downloading from an offsite location).
- The OttoFMS OData Proxy now works with the $metadata endpoint.
- Added a new environment variable PREVENT_DAPI_CALLOUT to turn off the callback to the servers hostname when using the DAPI or OData proxies. This is not turned on by default as there are features of the Data API that will break if this is turned off, primarily around the container data urls that get sent back in the Data API.
- Reverted a change made in OttoFMS 4.11.0 where the Admin API Proxy called out to the server hostname rather than directly to port 16001 as this was causing issues on servers where the server was unable to connect to its own hostname.
- Increased the default timeout for opening and closing files via fmsadmin from 10 to 20 seconds.
- Added a new env variable FMSADMIN_TIMEOUT_IN_SEC, which will override the default timeout of 20 seconds for fmsadmin commands.
- Fixed an issue where OttoFMS fails to rename files that include a non-unicode character.
- Fixed an error where some files would fail to upload when uploading multiple files.
...
- OttoFMS now uses the fmsadmin CLI to create backups rather than creating a FileMaker schedule. This is significantly faster (in testing, builds are up to 75% faster) than the admin api method was.
...
- Fixed an issue where builds would be removed from the build inbox based on only the success or failure of the final sub-deployment rather than the success of the sub-deployments in which the build was used. Builds will now only be removed from the inbox if all sub-deployments using the build succeed or they fail while the build is being downloaded (to avoid partial downloads causing issues).
...
- OttoFMS has a new tab for Builds, which displays a historical log of builds run on the server (similar to the deployments view). Builds can be deleted from the server, rerun, and their details (files, logs, times, etc.) can be viewed from a build detail page. Builds that are still in the server outbox can be downloaded and deleted from the server.
...
- OttoFMS now exposes an option in the start build endpoint to keep the files in the build unzipped. This can be used to run local deployments without zipping or to use builds as a simple programmatic way to run backups that you can script for other uses than deployments.
...
- Added more debug logging to backup creation, builds, schedules, all admin api calls from OttoFMS, and file opening and closing.
...
- The API Keys tab has been split into sub-tabs for the Data API and the Admin API.
...
- The Spotlight search (CMD+K) now has direct links to the sub-pages of the api keys, filemaker server, and settings pages.
...
- When using the "push build" feature, OttoFMS will now properly error out when the upload from the source server fails. This requires that both the source and destination servers be on version 4.8.0 to work.
...
- Added a new redirect url to OttoFMS. This redirect is at /otto/redirect/fm/[filename]/[script-name] and it will redirect the user to a filemaker file (using an fmp url) and it will run the specified script. It will also take all query parameters on the url and set them as variables when running the script. This can be used as a redirect for an OAuth flow or for other circumstances where you need to redirect a user to a FileMaker file and collect information from the url as you do.
...
- Fixed an issue where folders and sub-folders in the database folders which did not allow read access would cause deployments to fail.
...
- When a deployment completes successfully but has errors (ie. the post-deployment script fails, but it is not set to fail the entire deployment), the OttoFMS console will display a warning in the deployments table and the slack notification will include a warning count.
...
- OttoFMS on Linux now logs stderr and stdout to a stderr.log and a stdout.log file. This should make debugging errors on OttoFMS startup much simpler.
...
- Increased the default timeout time of pre-build and post-deployment scripts from 2 minutes to 1 hour. These values can be overridden using new env variables POST_DEPLOYMENT_SCRIPT_TIMEOUT_IN_SEC and PRE_BUILD_SCRIPT_TIMEOUT_IN_SEC.
...
- Files can now be renamed via the api and from the databases list in the ottofms console. This rename is does not currently handle external file references or other places where the filename is referenced.
...
- The extended privileges column in the databases table will now allow filtering on multiple privileges sets and will allow selection of all privileges sets available in the files rather than a specific set.
...
- the backup timeout time can now be overridden using the BACKUP_FILE_TIMEOUT_IN_SEC env variable. this defaults to 600 seconds (10 minutes).
...
- Added the closeFilesAfterBuild option to the deployment input, allowing deployment files to be closed after the build fetches fully rather than before. This should cut down on the file downtime if the build process takes a while.
...
- Added the fms extended privileges column to the FMS admin databases table. This column is hidden by default and can be revealed in the top right of the table.
...
- Fetched failed errors will no longer cause OttoFMS to crash unnecessarily
...
- fix issue with offsite backups on AWS regions other than us-east-1
...
- added link to the webhooks guide from the error if your database is not properly set up to receive webhooks
...
- a failure to keep the admin api token alive while running a deployment will now log an error
...
- deployments will now only log that you are logged into the source server if you are using the userpass authentication method since api keys and tokens do not need to log in.
...
- fixed issue with auto-register webhook toggle toggling the auto-register webhook behavior backwards
...
- fix admin API login error with keys
...
- databases are now sorted alphabetically in the fms admin tab
...
- fixed issue with FileMaker Server 20.3 preserved folder
