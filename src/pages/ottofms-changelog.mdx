...
- The webhook reciever now accepts GET requests. By default this will include all query parameters in the body sent to the reciever script, and the includeHeaders query option is still respected.
...
- Fixed an issue where the Additional Contianer Data Folder 2 would not be properly uploaded to offsite backups.
- Prevented a bug where webhooks without .fmp12 in the url would be saved separately from webhooks with .fmp12 in the url.
- The databases table on the filemaker server tab can now group files by status, client count, folder, sub folder, and privelage sets. This grouping will be saved off to your browser and carried across sessions.
