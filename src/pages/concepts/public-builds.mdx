...
<Steps>
### Use Otto Deploy to configure your build.
OttoDeploy's build interface walks you through the process of choosing the files to include you build, whether to clone or copy them, and how to run a script before the build to prepare your files.
...
Use OttoDeploy to download the build. Unzip the archive and extract the manifest.json file. Leave the archive intact. You will need it both the manifest.json file and the archive to distribute your build.
...
You place the zip archive and the manifest.json file next to each other on a webserver. You would use a structure like this to host your public builds.

<FileTree>
  <FileTree.Folder open={true} name="web-accessbile-folder" defaultOpen>
...
OttoDeploy and SimpleQ are both available as public builds.
