...
In Otto 3 Scheduled deployments where run by Migrator.fmp12. They were not part of the old migration defintion. That had some wierd contrainsts such as requiring the Migrator.fmp12 file to be hosted on the same server that was doing the migrtation.

Now the "Deplyment Definition" includes the scheduling. The Deployment is sent to the target server, and that server will schedule the deployment to run at the specified time. OttoDeploy ( the new Migrator )
doesn't need to be running anywhere specific.

## Deploments are Atomic
...
If you are a vertical market solution provider, or you want to incorporate a staging server into your setup or for any other more adanced setups you are going to want to use the Advanced interface. Amoung other things, the advanced interface allows you to use Sub-deployments.

## Sub-deployments

Turning on Advanced in OttoDeploy, gives you access to sub-deployments. Sub-deployments are a way to break up a deployment into smaller sets of files. The most common case for this is if you need to upgrade many copies of a single solution running on a server. For example, if you have 10 copies of a solution running on a server, you can create a subdeployment for each copy.
...
- Sub-deployments can use multiple different sources for the files, allowing you to run a set of 5 sub-deployments from 5 different sources, whereas in OttoFMS 3 you needed to run 5 seperate migrations.

### What sub-dployments can't do

Sub-dployments can't be scheduled seperately. If you need to schedule a sub-deployment seperately, you will need to create a seperate deployment for it.

Sub-deployments can't target different Destination Servers. A sub-deployment can use a different source for the files, but they will all use the same destination server. If you need to deploy to different destination servers, you will need to create a seperate deployment for each destination server.

## Relationship to Builds

<Callout type="info">
	One of the biggest changes between OttoFMS and OttoV3 is that you can now
	seperate the process of creating a build from the process of deploying a
