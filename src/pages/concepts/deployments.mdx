...
The source and the destination can be the same server. For example, you can migrate a file from file A to file B on the same server. If using the same server, unsure that you are not trying to migrate a file onto itself. This will cause the deployment to fail.
