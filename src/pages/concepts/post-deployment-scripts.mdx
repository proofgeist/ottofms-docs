...
[OttoDeploy](/ottodeploy) includes an option to run a Post Deployment script after a file has been deployed to a server. This script can be used to do any setup that is required after the file has been deployed.

The [OttoFMS Developer API](/apidoc) will let you pass an optional script to run after the file has been deployed. This script will be run after the file has been deployed and any data migration has completed.
